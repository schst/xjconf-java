...
public class ObjectValueConverter implements ValueConverter {
    ...
    public Object convertValue(Object[] values, Class<?>[] types, ClassLoader loader) throws ValueConversionException {
        ...
        if (instanceClass.isInterface()) {
            throw new ValueConversionException("Class " + className + " is an interface.");
        } else if (Modifier.toString(instanceClass.getModifiers()).contains("abstract")) {
            throw new ValueConversionException("Class " + className + " is abstract.");
        } else if (instanceClass.isAnnotation()) {
            throw new ValueConversionException("Class " + className + " is an annotation.");
         ...
         */
        try {
            typeConverter.doInstantiate(className, loader);
            typeConverter.setValues(values, types);
        } catch (Exception e) {
            throw new ValueConversionException("Trying to create an Object of Class " + className + " faild!", e);
        }
